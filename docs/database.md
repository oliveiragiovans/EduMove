...
Every teacher must belong to a school. Deactivating a teacher must not remove
previously registered assessments. `password_hash` receives a salted Argon2id value
generated by the service layer; cleartext passwords never reach the database.
...
contains 348 passing tests, every ORM model has been compared with the live MySQL
schema, and the class, student, assessment, motor-result, and postural-observation
workflows have been validated transactionally against MySQL. Secure teacher
registration, login, and password change have also been verified with rollback.
